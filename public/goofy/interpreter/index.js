...
const parse = (() => {
  ...
  function parseParametersExpr() {
    if (tokens[0].type === "parametersToken") {
      ...
      return {
        type: "parameterExpr",
        parameters: parseExpr().value.map(o => { return o.value })
      }
    }
  ...
  function parseReturnExpr() {
    ...
    if (value.type === "return" && tokens[0].type === "braceToken" && tokens[0].value === "open") {
      return {
        type: "returnExpr",
        block: parseExpr()
      };
  ...
  function parseFunctionCallExpr() {
    ...
    if (value.type === "identifier" && tokens[0].type === "braceToken" && tokens[0].value === "open") {
      return {
      ...
      case "braceToken":
        if (tokens[0].value === "open") {
          ...
          blockStmtRepeat: while (safetyLimit < 4096 && tokens[0] != null) {
            if (tokens[0].type === "braceToken" && tokens[0].value === "close") {
          ...
          if (safetyLimit >= 4096) {
            alert("ParsingError: Block statement too large or closing brace missing");
          }
          tokens.shift();
          return stmt;
        } break;
...
const interpreter = (() => {
    ...
    switch (stmt.type) {
      case "switchExpr": {
        ...
        for (let i = 0; i < cases.length; i ++) {
          ...
          if (evalExpr(currentCase.test.value[0]) === evalExpr(discriminant)) {
      ...
      case "variableAssignment": {
        if (variables[stmt.name] == null) {
          alert("InterpreterError: The name " + stmt.name + " does not exist in the current scope");
          return {};
        }
        variables[stmt.name] = stmt.value;
  ...
  return ast => {
    ...
    if (!Array.isArray(ast.statements)) {
      alert("InterpreterError: Unexpected AST statements list type " + typeof ast.statements);
      return;
    }
    lastOutput = "";
