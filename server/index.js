...
process.stdout.write(
  String.fromCharCode(27) + "]0;" + Config.WINDOW_NAME + String.fromCharCode(7)
);
...
let maintainloop = () => {
  ...
  if (
    !naturallySpawnedBosses.length &&
    bossTimer++ > Config.BOSS_SPAWN_COOLDOWN
  ) {
    bossTimer = -Config.BOSS_SPAWN_DURATION;
    let selection =
        Config.BOSS_TYPES[
          ran.chooseChance(
            ...Config.BOSS_TYPES.map((selection) => selection.chance)
          )
        ],
  ...
  if (!global.arenaClosed && bots.length < Config.BOTS) {
    ...
    let color = Config.RANDOM_COLORS
