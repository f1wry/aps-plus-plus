...
exports.getDistanceSquared = (p1, p2) =>
...
exports.listify = (list) => {
  ...
  for (let [i, item] of list.entries()) {
    if (typeof item !== "string")
      throw Error(`Item #${i + 1} (${item} of list is not a string.`);
    output += i !== list.length - 1 ? `${item}, ` : `and ${item}`;
  }
  return output;
};

exports.angleDifference = (a1, a2) =>
  ((((a2 - a1) % (2 * Math.PI)) + Math.PI * 3) % (2 * Math.PI)) - Math.PI;

exports.interpolateAngle = (angle, desired, step) =>
  angle + exports.angleDifference(angle, desired) * step;
...
exports.getJackpot = (x) =>
  x > 39450 ? Math.pow(x - 26300, 0.85) + 26300 : x / 1.5;
