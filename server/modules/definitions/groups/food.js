...
for (let tier = 0; tier < 6; tier++) {
  for (let poly in polyNames) {
    ...
    for (let shiny in shinyNames) {
      ...
      Class[`laby${tier}${food}`] = Class[ // backwards compatability, DO NOT ADD A SEMICOLON HERE. javascript is funny about whitespace characters :))))))
        `laby_${poly}_${tier}_${shiny}_0`
      ] = makeLaby(
        Class[food],
        tier,
