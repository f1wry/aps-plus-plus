...
class Train {
  ...
  loop() {
    ...
    for (let team of teams) {
      let train = train_able
        .filter((r) => r.team === team && !r.invuln)
      ...
      for (let [i, player] of train.entries()) {
        ...
        player.velocity.x =
          util.clamp(train[i - 1].x - player.x, -90, 90) * player.damp * 2;
        player.velocity.y =
          util.clamp(train[i - 1].y - player.y, -90, 90) * player.damp * 2;
      }
    }
  }
}
