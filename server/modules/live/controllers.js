let compressMovementOffsets = [
  ...
  wouldHitWall = (me, enemy) => {
    ...
    for (let i = 0; i < walls.length; i++) {
      ...
      if (
        util.getDistanceSquared(inclusionCircle, crate) >
        (inclusionCircle.radius + crate.hitboxRadius) ** 2
      )
        continue;
      ...
      for (let j = 0; j < crate.hitbox.length; j++) {
        ...
        if (
          collisionLineLine(
            ...
            enemy.x,
            enemy.y,
            crate.x + hitboxLine[0].x,
            crate.y + hitboxLine[0].y,
            crate.x + hitboxLine[1].x,
            crate.y + hitboxLine[1].y
          )
        )
          return true;
      }
      ...
      if (comp > 0) {
        if (input.goal) {
          let goalDist = Math.sqrt(
            range / (input.goal.x ** 2 + input.goal.y ** 2)
          );
          goal = {
...
class io_formulaTarget extends IO {
  ...
  think() {
    ...
    let angle =
      this.originAngle +
      this.formula((this.frame += 1 / Config.runSpeed), this.body);
    return {
      goal: {
...
class io_snake extends IO {
  ...
  think(input) {
    ...
    // Follow the sin wave
    this.body.x = util.lerp(
      this.body.x,
      this.startX + trueWaveX,
      this.velocityMagnitude
    );
    this.body.y = util.lerp(
      this.body.y,
      this.startY + trueWaveY,
      this.velocityMagnitude
    );
    // Accelerate after spawning
    this.velocityMagnitude = Math.min(
      1,
      this.velocityMagnitude + 0.018 / Config.runSpeed
    );
  }
}
