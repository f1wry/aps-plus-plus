...
class Entity extends EventEmitter {
  ...
  define(defs, emitEvent = true) {
    ...
    if (set.UPGRADE_COLOR)
      this.upgradeColor = new Color(set.UPGRADE_COLOR).compiled;
    if (set.GLOW != null) {
      this.glow = {
  ...
  refreshBodyAttributes() {
    ...
    this.acceleration =
      (accelerationMultiplier * Config.runSpeed * this.ACCELERATION) /
      speedReduce;
    if (this.settings.reloadToAcceleration) this.acceleration *= this.skill.acl;
    this.topSpeed =
      (topSpeedMultiplier * Config.runSpeed * this.SPEED * this.skill.mob) /
      speedReduce;
  ...
  bindToMaster(position, bond, isInvulnerable) {
    ...
    if (
      this.facingType[0].includes("Target") ||
      this.facingType[0].includes("Speed")
    ) {
    ...
    switch (type) {
      ...
      case "desmos":
        ...
        if (this.waveAngle == null) {
          ...
          this.velocity.x =
            ((5 + this.velocity.length * (amount + 2)) *
              Math.cos(this.waveAngle)) /
            7;
          this.velocity.y =
            ((5 + this.velocity.length * (amount + 2)) *
              Math.sin(this.waveAngle)) /
            7;
        }
        let waveX =
          this.maxSpeed *
  ...
  face() {
    ...
    switch (type) {
      ...
      case "turnWithSpeed":
        this.facing +=
          ((this.velocity.length / 90) * Math.PI) / Config.runSpeed;
        break;
      case "spin":
      ...
      case "looseWithMotion":
        this.facing = util.interpolateAngle(
          this.facing,
          this.velocity.direction,
          Config.runSpeed / (args.speed ?? 4)
        );
        break;
      case "withTarget":
      case "toTarget":
      ...
      case "bound":
        let angleToTarget,
          ...
          slowness = this.settings.mirrorMasterAngle
            ? 1
            : (args.slowness ?? 4) / Config.runSpeed;
        if (this.control.main) {
          angleToTarget = Math.atan2(t.y, t.x);
          angleDiff = Math.abs(
            util.angleDifference(angleToTarget, this.firingArc[0])
          );
        ...
        this.facing = util.interpolateAngle(
          this.facing,
          angleToTarget,
          Math.min(1, 1 / (slowness * Math.min(1, angleDiff)))
        );
        break;
    }
    ...
    if (this.facingLocked) {
      ...
      this.vfacing =
        util.angleDifference(oldFacing, this.facing) * Config.runSpeed;
    }
  }
  takeSelfie() {
    ...
    if (!this.settings.canGoOutsideRoom) {
      if (Config.ARENA_TYPE === "circle") {
        ...
        if (dist > room.width / 2) {
          let strength =
            ((dist - room.width / 2) * Config.ROOM_BOUND_FORCE) /
            (Config.runSpeed * 750);
  ...
  contemplationOfMortality() {
    ...
    if (this.isDead()) {
      ...
      let killCountEntries = Object.entries(killCounts).map(
        ([name, count], i) => name
      );
      for (let i = 0; i < killCountEntries.length; i++) {
        killText +=
          killCounts[killCountEntries[i]] == 1
            ? util.addArticle(killTools[i].label)
            : killCounts[killCountEntries[i]] + " " + killCountEntries[i] + "s";
