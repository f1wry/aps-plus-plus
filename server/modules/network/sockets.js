...
function incoming(message, socket) {
  ...
  switch (m.shift()) {
    ...
    case "L":
      ...
      if (
        player.body.skill.level < Config.LEVEL_CHEAT_CAP ||
    ...
    case "M":
      ...
      chats[id].unshift({
        message,
        expires: Date.now() + Config.CHAT_MESSAGE_DURATION,
      });
...
const spawn = (socket, name) => {
  ...
  body.name = name;
  body.sendMessage = (content, displayTime = Config.MESSAGE_DISPLAY_TIME) =>
...
let leaderboard = new Delta(7, (args) => {
  ...
  for (let i = 0; i < 10 && list.length; i++) {
    ...
    let entry = list[top];
    let color =
      args.length && args[0] == entry.id
        ? "10 0 1 0 false"
        : entry.color.compiled;
    topTen.push({
      id: entry.id,
      data: [
        Config.MOTHERSHIP_LOOP
...
setInterval(() => {
  ...
  for (let socket of clients) {
    ...
    if (time - socket.statuslastHeartbeat > Config.maxHeartbeatInterval)
