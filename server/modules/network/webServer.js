...
if (Config.host === "localhost") {
  util.warn(
    `config.host is just "localhost", are you sure you don't mean "localhost:${Config.port}"?`
  );
}
if (
  Config.host.match(/localhost:(\d)/) &&
  Config.host !== "localhost:" + Config.port
) {
  util.warn(
    "config.host is a localhost domain but its port is different to config.port!"
  );
}
