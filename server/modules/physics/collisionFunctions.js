...
function advancedcollide(my, n, doDamage, doInelastic, nIsFirmCollide = false) {
  let tock = Math.min(my.stepRemaining, n.stepRemaining),
    ...
    direction = new Vector(
      (n.x - my.x) / difference.length,
      (n.y - my.y) / difference.length
    ),
  ...
  let reductionFactor = 1,
    ...
    depth = {
      _me: util.clamp(
        (combinedRadius - difference.length) / (2 * my.size),
        0,
        1
  ...
  if (doDamage) {
    ...
    if (!bail) {
      ...
      if (n.settings.ratioEffects) {
        damage._n *= Math.min(
          1,
          Math.pow(Math.max(n.health.ratio, n.shield.ratio), 1 / n.penetration)
        );
      }
  ...
  if (nIsFirmCollide < 0) {
    ...
    if (doInelastic && my.settings.motionEffects && n.settings.motionEffects) {
      elasticity *=
        savedHealthRatio._me / pen._me.sqrt + savedHealthRatio._n / pen._n.sqrt;
    } else {
      elasticity *= 2;
    }
    let spring =
      ...
      modifiers = {
        _me:
          ((Config.KNOCKBACK_CONSTANT * my.pushability) / my.mass) *
          deathFactor._n,
        _n:
          ((Config.KNOCKBACK_CONSTANT * n.pushability) / n.mass) *
          deathFactor._me,
      };
...
function mazewallcollide(wall, bounce) {
  if (
    bounce.god === true ||
    bounce.passive === true ||
    bounce.isArenaCloser ||
    bounce.master.isArenaCloser
  )
    return;
  ...
  if (
    ...
    bounce.accel.x -=
      (bounce.x + bounce.size - wall.x + trueWallSize) * bounceBy;
  } else if (
    (right && !top && !bottom) ||
    ...
    bounce.accel.y -=
      (bounce.y + bounce.size - wall.y + trueWallSize) * bounceBy;
  } else if (
    (bottom && !left && !right) ||
