class CollisionGrid {
  ...
  addToGrid(instance) {
    ...
    if (!this.grid[cellY] || !this.grid[cellY][cellX])
      return this.outOfBoundObjects.push(toAdd);
    ...
    for (let i = 0; i < this.totalInstances.length; i++) {
      ...
      if (cell.length > 1) {
        for (let j = 0; j < cell.length; j++) {
          ...
          if (!pairs.includes(pair) && hit && instance.id !== other.id)
            pairs.push(pair);
        }
      }
    }
...
class QuadTree {
  ...
  hitDetection(object, other) {
    return (
      Math.sqrt(
        Math.pow(other.x - object.x, 2) + Math.pow(other.y - object.y, 2)
      ) <
    ...
    for (let i = 0; i < closeBy.length; i++) {
      ...
      let pair =
        object.size > other.size
          ? [object.id, other.id]
          : [other.id, object.id];
      if (hit && !pairs.includes(pair) && object.id !== other.id)
        pairs.push(pair);
    }
    return pairs;
  }
