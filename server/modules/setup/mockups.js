...
function sizeEntity(entity, x = 0, y = 0, angle = 0, scale = 1) {
  ...
  if (entity.shape < 3 || entity.shape >= sidesMax) {
    ...
    for (let i = 0; i < numSides; i++) {
      ...
      endPoints.push([
        x + Math.cos(theta) * scale * lazyRealSizes[numSides],
        y + Math.sin(theta) * scale * lazyRealSizes[numSides],
      ]);
    }
  }
...
let mockupCount = 0;
let timeDist = 1000;
let totalMockupsCount = Object.keys(Class).length;
for (let k in Class) {
  try {
    // Log it
    mockupCount ++;
    ...
    temptank.destroy();
    if (Date.now() - lastLogTime > timeDist) {
        lastLogTime = Date.now();
        console.log("[" + ((lastLogTime - mockupsLoadStartTime) / 1000).toFixed(3) + "] " + mockupCount + "/" + totalMockupsCount + " mockups have finished loading.");
        timeDist *= 1.2;
    }
  } catch (error) {
...
console.log(
  "Finished compiling " + mockupData.length + " classes into mockups."
);
console.log(
  "Mockups generated in " +
    (mockupsLoadEndTime - mockupsLoadStartTime) +
    " milliseconds.\n"
);
console.log(
  "Server loaded in " +
    (mockupsLoadEndTime - serverStartTime) +
    " milliseconds.\n"
);
mockupsLoaded = true;
