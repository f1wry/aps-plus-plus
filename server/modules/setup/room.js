...
global.room = {
  ...
  topPlayerID: -1,
  partyHash: Number(
    (((Math.random() * 1000000) | 0) + 1000000).toString().replace("0.", "")
  ),
  spawnableDefault: [],
  center: {},
  spawnable: {},
...
Object.defineProperties(room, {
  tileWidth: {
    ...
    set: (v) => (Config.TILE_WIDTH = v),
  },
  tileHeight: {
    get: () => Config.TILE_HEIGHT,
    set: (v) => (Config.TILE_HEIGHT = v),
  },
  width: {
    get: () => room.xgrid * Config.TILE_WIDTH,
    set: (v) => (Config.TILE_WIDTH = v / room.xgrid),
  },
  height: {
    get: () => room.ygrid * Config.TILE_HEIGHT,
    set: (v) => (Config.TILE_HEIGHT = v / room.ygrid),
  },
});
...
Object.defineProperties(room.center, {
  x: {
    ...
    set: (v) => (Config.TILE_WIDTH = (v * 2) / room.xgrid),
  },
  y: {
    get: () => (room.ygrid * Config.TILE_HEIGHT) / 2,
    set: (v) => (Config.TILE_HEIGHT = (v * 2) / room.ygrid),
  },
});
...
room.isInRoom = (location) => {
  if (Config.ARENA_TYPE === "circle") {
    return (
      (location.x - room.center.x) ** 2 + (location.y - room.center.y) ** 2 <
      room.center.x ** 2
    );
  }
  return (
...
class TileEntity {
  constructor(tile, loc) {
    if (!(tile instanceof Tile)) {
      throw new Error(
        `The cell at ${loc.x},${loc.y} in the room setup is not a Tile object!` +
          ("string" == typeof tile
            ? " But it is a string, which means you probably need to update your room setup!"
            : " But it is of type " + typeof tile)
      );
    }
    let gridLoc = (this.gridLoc = {
      x: parseFloat(loc.x),
      y: parseFloat(loc.y),
    });
