let dominatorTypes = [
    ...
    o.on("dead", () => {
      ...
      if (team === TEAM_ENEMIES) {
        ...
        killer = killer
          ...
          : { team: TEAM_ROOM, color: Config.MODE === "tdm" ? 3 : 12 };
        ...
        for (let player of sockets.players) {
          if (player.body && player.body.team === newTeam) {
            player.body.sendMessage(
              "Press F to take control of the dominator."
            );
          }
        }
  ...
  makeDefenderDominator = (tile, mainTeam, team, aliveDef) => {
    ...
    o.on("dead", () => {
      ...
      makeDefenderDominator(
        tile,
        mainTeam,
        team === mainTeam ? TEAM_ENEMIES : mainTeam,
        aliveDef
      );
      sockets.broadcastRoom();
    });
...
let dominatorBlue = new Tile({
    init: (tile) => makeDefenderDominator(tile, TEAM_BLUE, TEAM_BLUE),
  }),
  dominatorGreen = new Tile({
    init: (tile) => makeDefenderDominator(tile, TEAM_GREEN, TEAM_GREEN),
  }),
  dominatorContested = new Tile({
    init: (tile) => spawn(tile, TEAM_ENEMIES, getTeamColor(TEAM_ENEMIES)),
  }),
  sanctuaryBlue = new Tile({
    init: (tile) =>
      makeDefenderDominator(tile, TEAM_BLUE, TEAM_BLUE, "sanctuaryTier1"),
  }),
  sanctuaryGreen = new Tile({
    init: (tile) =>
      makeDefenderDominator(tile, TEAM_GREEN, TEAM_GREEN, "sanctuaryTier1"),
  }),
  sanctuaryContested = new Tile({
    init: (tile) =>
      spawn(tile, TEAM_ENEMIES, getTeamColor(TEAM_ENEMIES), "sanctuaryTier1"),
  });
