let pickFromChanceSet = (set) => {
  ...
  normal = new Tile({
    ...
    tick: (tile) => {
      if (++tile.data.foodSpawnCooldown > Config.FOOD_SPAWN_COOLDOWN) {
        ...
        if (
          tile.data.foodCount < Config.FOOD_CAP &&
          Math.random() < Config.FOOD_SPAWN_CHANCE
        ) {
  ...
  nestTick = (tile) => {
    if (++tile.data.enemySpawnCooldown > Config.ENEMY_SPAWN_COOLDOWN_NEST) {
      ...
      if (
        tile.data.enemyCount < Config.ENEMY_CAP_NEST &&
        Math.random() < Config.ENEMY_SPAWN_CHANCE_NEST
      ) {
    ...
    if (++tile.data.foodSpawnCooldown > Config.FOOD_SPAWN_COOLDOWN_NEST) {
      tile.data.foodSpawnCooldown = 0;
      if (
        tile.data.foodCount < Config.FOOD_CAP_NEST &&
        Math.random() < Config.FOOD_SPAWN_CHANCE_NEST
      ) {
